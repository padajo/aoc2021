...
school.age_school(days=6, display=False)

print('After 86 days there are:', len(school.school), 'fish')

# THIS FAILS
# It gets too big as an array, so this means processing it as text, and leaving it as text between iterations
'''
n = 10
days = 86
for d in range(days, 256 + n, +n):
    school.age_school(days=n, display=False)
    print('After', (days + n),'days there are:', len(school.school), 'fish')
    print(d)
'''
