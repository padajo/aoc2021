...
for n in range(0,len(bingodata['calls'])):
    ...
    if(keep_calling):
        ...
        for i in range(0, len(bingodata['cards'])):
            if not i in winning_card_index:
                card = bingodata['cards'][i]
                is_winner = is_card_a_winner(card, called)
                #print(is_winner)
                if is_winner:
                    #show_all_cards(bingodata, called)
                    display_card(card, called)
                    winning_card_index.append(i)
                    print('CARD SCORE:', score_winning_card(card, called))
                    if len(winning_card_index) == len(bingodata['cards']):
                        # end the loop
                        keep_calling = False

print('Index of winning cards:', winning_card_index)

print('\n*** LAST CARD DETAILS ***\n')
last_card = bingodata['cards'][winning_card_index[len(winning_card_index)-1]]
display_card(last_card, called)
print('CARD SCORE:', score_winning_card(card, called))
